// implementation for GCC, Windows-mingwin
...
bool dir::setd(std::string s) { return !chdir(s.c_str()); }
std::string dir::getd()
{
    int size = 100;
...
bool dir::make(std::string s)
{
    return !mkdir(s.c_str());
}

bool dir::remove(std::string s)
{
    return !rmdir(s.c_str());
}

bool dir::rename(std::string old, std::string n)
{
