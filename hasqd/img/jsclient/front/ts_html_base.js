...
function widGetHTMLSpanImg(id, func) {
    ...
    if (func) {
        r += '<span id="' + id + '" onclick="' + func +
            '"style="text-align:center;"><img src=""></img></span>\n';
    } else {
        r += '<span id="' + id + '" style="text-align:center;">' +
            '<img src=""></img></span>\n';
    }
...
function widGetHTMLSpan(id, func) {
    ...
    if (func) {
        r += '<span style="text-align:center;" id="' + id +
            '" onclick="' + func + '"></span>\n';
    } else {
        r += '<span style="text-align:center;" id="' +
            id + '"></span>\n';;
    }
...
function widGetHTMLMessageBox(obj) {
    ...
    if (obj) {
        r += '<span class="span-message-box" onclick="' +
            'widHelpMessageBox($(this))">' + obj + '</span>\n';
    }
...
function widGetHTMLTrTextarea(id0, id1, oninput, style, attr) {
    ...
    r += '<textarea id="' + id1 + '" oninput="return ' + oninput +
        '" style="' + style + '" ' + attr + '></textarea>\n';
    r += '</tr>\n';
...
function widGetHTMLTrTdButton(isTr, id, htmlClass, onclick, img) {
    ...
    r += '<button id="' + id + '" class="' + htmlClass +
        '" onclick="return ' + onclick + '">' + img + '</button>\n';
...
function widGetHTMLInitArea() {
    ...
    var oninput = 'widTokTextOninput($(this), 500)';
...
function widGetHTMLTokenHash() {
    ...
    var body = function () {
        ...
        r += '<td id="td_reload_button" title="' +
            gTooltip.refresh_token + '"/>\n';
        r += widGetHTMLSpanImg('span_reload',
            'widReloadButtonOnclick($(this))');
        ...
        r += '<label id="label_file_upload" for="input_file_upload">File' +
            '</label>\n';
        r += '<input id="input_file_upload" type="file" onchange="return' +
            ' widLoadFiles(this.files)"></input>';
...
function widGetHTMLMasterKey() {
    ...
    r += '<tr>\n';
    r += '<td/>\n';
    r += widGetHTMLSpanImg('span_password_eye',
        'widPasswordEyeClick($(this))');
    r += '<td/>\n';
    r += '<input oninput="widPasswordOninput($(this));" id="input_password"' +
        ' type="password" class="password" placeholder=' +
        '"Enter token master key" required/>\n';
    r += '<td/>\n';
...
function widGetHTMLTabsArea(tabs) {
    ...
    r += '<tr>\n';
    r += '<td nowrap>\n';
    r += '<table id="table_main_area" style="border: 1px solid #DDDDDD;' +
        ' width: 100%;">\n';
    r += '<tr>\n';
...
function widGetHTMLButtonsArea() {
    ...
    var f = function (tabId, img) {
        ...
        r += '<tr>\n';
        r += '<td/>\n';
        r += '<button class="tab-btn-up" onclick="' +
            'return widTabButtonClick($(this),' + tabId +
            ')">' + img + '</button>\n';
        r += '</tr>\n';
...
function widGetHTMLTabs(items, id, classId) {
    ...
    for (let i = 0; i < items.length; i++) {
        r += '\t<div id="div_' + id + '_' + i + '">' + items[i].data +
            '</div>\n';
    }
...
function widGetHTMLCreateTab() {
    ...
    r += widGetHTMLTrTdButton(1, 'button_create', 'show-keys-btn-up',
        'widCreateButtonClick($(this))', imgBtnCreate);
    r += '</table>\n';
...
function widGetHTMLSetDataTab() {
    ...
    r += widGetHTMLTrTextarea('', 'textarea_set_data',
        'widSetDataTextareaOninput($(this))', '', 'type="text" wrap="off"');
    r += '<tr>';
    r += '<td/>';
    ...
    r += widGetHTMLTrTdButton(0, 'button_set_data', 'button-disabled',
        'widSetDataButtonClick($(this))', imgBtnData);
    r += '<td/>';
    r += '</tr>';
    r += '</table>';
    r += '<tr>';
...
function widGetHTMLShowKeysButtons() {
    ...
    r += '<table>\n';
    r += '<tr>\n';
    r += widGetHTMLTrTdButton(0, 'button_show_instant', 'show-keys-btn-up',
        'widShowInstantButtonClick($(this))', imgBtnInstantKeys);
    r += widGetHTMLTrTdButton(0, 'button_show_on_hold', 'show-keys-btn-up',
        'widShowOnHoldButtonClick($(this))', imgBtnOnHoldKeys);
    r += widGetHTMLTrTdButton(0, 'button_show_release', 'show-keys-btn-up',
        'widShowReleaseButtonClick($(this))', imgBtnReleaseKeys);
    r += '</tr>\n';
    r += '</table>\n';
    r += '</tr>\n';
...
function widGetHTMLReceiveTab() {
    ...
    r += widGetHTMLTrTdButton(1, 'button_receive', 'show-keys-btn-up',
        'widReceiveButtonClick($(this))', imgBtnReceiveKeys);
    r += '</table>\n';
...
function widGetHTMLSearchTab() {
    ...
    r +=
        '<table id="table_search_tab" style="width:100%; margin: 0;" border="0">\n';
    r += '<tr>\n';
    r += '<td id="td_search_tabs_buttons" rowspan="2"/>\n';
    r += widGetHTMLSearchButtons();
    r += widGetHTMLTrTdSubtitle(0, 'Search for tokens', 'td-tab-subtitle',
        'colspan="3"');
    r += '</tr>\n';
    r += '<tr>\n';
...
function widGetHTMLSearchInits() {
    ...
    r += '<button id="button_search" class="show-keys-btn-up" ' +
        'onclick="return widSearchButtonClick($(this));">' + imgBtnStart +
        '</button>\n';
    r += '</tr>\n';
    r += '<tr>\n';
...
function widGetHTMLSearchButtons() {
    ...
    var toButton = function (tabId, htmlClass, img, name, spanId) {
        var r = '';
        var onclick =
            'onclick="return widSearchResultsTabsClick($(this),' +
            tabId + ')" ';
        ...
        r += '<tr>\n';
        r += '<td/>';
        r += '<button ' + onclick + htmlClass + '>' + img + '<br/>' +
            toSpan(span) + '<br/>' + name + '</button>';
        r += '</tr>\n';
    ...
    r += '<table>\n';
    r += toButton(0, 'search-tabs-buttons-active', imgBtnSearchMine, 'Mine',
        'span_search_mine');
...
function widGetHTMLLogArea() {
    ...
    r += '<table style="width: 100%">\n';
    r += '<tr>\n';
    r += '<td class="td-subscribe" style="text-align: left;' +
        ' width: 80px;"/>\n';
