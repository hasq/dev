...
function engCompareTokens(existingRec, expectingRec) {

    if ((existingRec.n === expectingRec.n) && (existingRec.s === expectingRec.s) && (existingRec.g === expectingRec.g) && (existingRec.o === expectingRec.o)) {
        return true;
    } else
        return false;
}
...
function engAddVTLItem(data, idx, table, cmdList){
    ...
    if (data === 'IDX_NODN') {
        ...
        if (engCompareTokens(existingRec, expectingRec)) {
