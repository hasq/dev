...
try
{
    ...
    cout << "\nDB Wizard, Hasq Technology Pty Ltd (C) 2013-2015\n\n";
    cout << "Conventions: square brackets contain options, round brackets - default values\n";
    cout << "Enter hash type for the database [md5, r16, s22, s25, wrd]: ";
    ...
    cout << "Enter records' data size limit [-1 - unlimited, suffix: b,K,M,H] (5H): ";
    string s_limit;
    getline(cin, s_limit);
    if ( s_limit.empty() ) s_limit = "5H";

    // calculate alternative name
    ...
    cout << "Magic          :   " << magic << "\n";
    cout << "Number of G's  :   " << nG << "\n";
    cout << "Slice size     :   " << i_sliceSize << " Kb\n";
    cout << "Thinness       :   " << i_thin << "\n";
    cout << "Data limit     :   " << s_limit << "\n";
    cout << "Alt name       :   " << alt << "\n";

    cout << "\nAre the above values correct? (y):";
    string yn;
    getline(cin, yn);
    if ( yn.empty() ) yn = "y";
    if ( yn != "y" )
    {
        cout << "No config file is created. Bye!\n";
        return 0;
    }
