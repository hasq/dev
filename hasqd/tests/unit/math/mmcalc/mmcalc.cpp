...
// Mazonka Modular Calculator 2013-2015
...
{
    std::cout << "Fatal error: (" << line << " )" << e.str() << "\n";
    return 2;
}
catch (...)
{
    std::cout << "Unknown error (while reading line " << line << ")\n";
    return 3;
}
...
Vtoken token_buf;
void putback(Token t) { token_buf.push_back(t); }
...
Token input_token_nodic()
{
    if ( !token_buf.empty() ) { Token t = token_buf.back(); token_buf.pop_back(); return t; }
...
read:
    ...
    {
        comm = false;
        if ( interactive ) std::cout << (inside_expr ? "-" : "") << "> ";
        line++;
        goto read;
    }
